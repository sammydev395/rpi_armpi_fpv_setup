#!/usr/bin/python3
# coding=utf8
# 第12章 ROS+OpenCV课程\1.AI视觉识别\第2课 颜色识别实验(12. Ros+OpenCV course\1. AI vision recognition\Lesson 2 color recognition)
import os
import cv2
import math
import time
...
#设置扩展板的RGB灯颜色使其跟要追踪的颜色一致(set the RGB light on the expansion color match the tracking color)
def set_rgb(color):
    if color == "red":
...
def getAreaMaxContour(contours):
    ...
    for c in contours:  # 历遍所有轮廓(iterate through all contour)
        contour_area_temp = math.fabs(cv2.contourArea(c))  # 计算轮廓面积(calculate contour area)
        if contour_area_temp > contour_area_max:
            contour_area_max = contour_area_temp
            if contour_area_temp > 300:  # 只有在面积大于300时，最大面积的轮廓才是有效的，以过滤干扰(only when contours with an area greater than 300 are considered valid, and it is used to filter out interference)
                area_max_contour = c
