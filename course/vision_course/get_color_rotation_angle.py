#!/usr/bin/python3
# coding=utf8
# 第8章 ArmPi FPV逆运动学基础及实战应用课程/第6课 色块角度识别(8. ArmPi FPV inverse kinematics basics & application/Lesson 6 color block angle recognition)
import os
import cv2
import math
import time
...
#设置扩展板的RGB灯颜色使其跟要追踪的颜色一致(set the color of the RGB light on the expansion board to match the tracking color)
def set_rgb(color):
    if color == "red":
...
def run(img):
    ...
    if max_area > 500:  # 有找到最大面积(the largest area is found)
        ...
        cv2.putText(img, 'angle: %s'%str(angle), (10, img.shape[0] - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.65, range_rgb[detect_color], 2)
    else:
        detect_color = 'None'
        set_rgb(detect_color)    
        cv2.putText(img, 'angle: None', (10, img.shape[0] - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.65, range_rgb[detect_color], 2)
