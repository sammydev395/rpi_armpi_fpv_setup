...
class ROS_Camera(QThread):
    ...
    def enter(self):
        self.running = True
        self.image_sub = rospy.Subscriber(self.image_topic, Image, self.get_ros_image)  # 订阅目标检测节点(describe target detection node)
    ...
    def get_ros_image(self, ros_image):
        self.image = np.ndarray(shape=(ros_image.height, ros_image.width, 3), dtype=np.uint8, buffer=ros_image.data)  # 将自定义图像消息转化为图像(convert the custom image information to image)
